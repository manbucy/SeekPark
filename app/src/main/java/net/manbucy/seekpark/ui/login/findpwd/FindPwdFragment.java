...
public class FindPwdFragment extends BaseFragment implements FindPwdContract.View {
    private FindPwdContract.Presenter findPwdPresenter;
    @BindView(R.id.toolbar)
    Toolbar toolbar;
    @BindView(R.id.fpf_input_phone)
    TextInputLayout phoneInputLayout;
    private EditText phone;
    @BindView(R.id.fpf_button_send_verify)
    Button verifyButton;
    @BindView(R.id.fpf_input_verify)
    TextInputLayout verifyInputLayout;
    private EditText verify;
    @BindView(R.id.fpf_input_password)
    TextInputLayout passwordInputLayout;
    private EditText password;
    @BindView(R.id.fpf_input_password_again)
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View root = inflater.inflate(R.layout.findpwd_fragment, container, false);
    ...
    @OnClick(R.id.fpf_button_send_verify)
    public void sendVerify() {
        if (!isPhone) {
            showInputErrorMsgs(Constant.InputLayoutEntry.PHONE_INPUT, "请输入正确的手机号码");
        } else {
            findPwdPresenter.sendVerifyCode(phone.getText().toString());
        }
    }

    @OnClick(R.id.fpf_button_reset_password)
    public void resetPassword() {
        checkEmpty();
        if (isVerify) {
            findPwdPresenter.resetPassword(password.getText().toString(), verify.getText().toString());
        }
    }
    ...
    public void onDestroy() {
        super.onDestroy();
        findPwdPresenter.detachView(this);
    }
}
